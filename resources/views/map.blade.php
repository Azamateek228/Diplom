    ...
    <div class="map-page">
        ...
        @if (isset($cities) && $cities->count())
            <div class="route-cities-list mt-4">
                ...
                <ol class="route-city-list">
                    @foreach ($cities as $city)
                        <li class="route-city-row {{ $currentCity && (int) $currentCity->id === (int) $city->id ? 'active-route-city' : '' }}">
                            <span class="route-city-order">{{ $loop->iteration }}</span>
                            <span class="route-city-name">{{ $city->name }}</span>
                            <span class="route-city-badges">
                                @if ($currentCity && (int) $currentCity->id === (int) $city->id)
                                    <span class="route-city-badge route-city-badge-current">текущий город</span>
                                @endif
                                @if (($city->votes_count ?? 0) > 0)
                                    <span class="route-city-badge route-city-badge-votes">{{ $city->votes_count }} голос(ов)</span>
                                @endif
                            </span>
                        </li>
                    @endforeach
